...
BEGIN
  ...
  */
  ...
  SET __outputtable = 'wfproc_api_c3b932854a8512b2_out_' || SUBSTRING(TO_HEX(MD5('')), 1, 16);
  SET __outputtablefqn = 'carto-dw-ac-8l15x5rr.workflows_temp_ernesto_sdscnyc_9fdb3fb4.wfproc_api_c3b932854a8512b2_out_' || SUBSTRING(TO_HEX(MD5('')), 1, 16);
  BEGIN
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_2249aad783a8f582_result`
    AS
      SELECT ST_GEOGFROMTEXT(`area_of_interest`) as geom;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_f79a822ab073195b_match`
    AS
      WITH withids AS (
        SELECT
          __source.*,
      ...
      repeated AS (
        ...
        FROM
          withids __source,
          `WORKFLOW_c3b932854a8512b2_2249aad783a8f582_result` __filter
        WHERE ST_INTERSECTS(
    ...
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_34be8113334ac175_result`
    AS
      SELECT * REPLACE (SAFE_CAST(datetime
      AS DATETIME) AS datetime)
      FROM `WORKFLOW_c3b932854a8512b2_f79a822ab073195b_match`;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_eef1da543ae2b138_result`
    AS
      SELECT * from `WORKFLOW_c3b932854a8512b2_34be8113334ac175_result` 
      WHERE
      datetime > '2024-10-05 00:00:00'
      AND
    ...
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_a12f80235b16f971_result`
    AS
      SELECT
        *,
        CASE
        ...
        ELSE 'After'
        END AS period
      FROM `WORKFLOW_c3b932854a8512b2_eef1da543ae2b138_result`;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_f0c6a1fb8fd9b694_result`
    AS
      SELECT period,
        SUM(vol_joined) vol_joined_sum
      FROM `WORKFLOW_c3b932854a8512b2_a12f80235b16f971_result`
      GROUP BY period;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_940c7a579e57ad2d_match`
    AS
      WITH withids AS (
        SELECT
          __source.*,
    ...
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_6bdcc5ed7d609f0f_match`
    AS
      WITH withids AS (
        SELECT
          __source.*,
    ...
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_827a2bba09ef8d8b_result`
    AS
      SELECT * REPLACE (SAFE_CAST(crash_datetime
      AS DATETIME) AS crash_datetime)
      FROM `WORKFLOW_c3b932854a8512b2_940c7a579e57ad2d_match`;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_da76a4b7df1b54b5_result`
    AS
      SELECT * REPLACE (SAFE_CAST(datetime
      AS DATETIME) AS datetime)
      FROM `WORKFLOW_c3b932854a8512b2_6bdcc5ed7d609f0f_match`;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_715facbd93094fcc_result`
    AS
      SELECT * from `WORKFLOW_c3b932854a8512b2_827a2bba09ef8d8b_result` 
      WHERE
      crash_datetime > '2024-10-05 00:00:00'
      AND
    ...
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_8b9afe49df58194a_result`
    AS
      SELECT * from `WORKFLOW_c3b932854a8512b2_da76a4b7df1b54b5_result` 
      WHERE
      datetime > '2024-10-05 00:00:00'
      AND
      datetime < '2025-04-05 00:00:00';
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_d38aaa5f086ae0f9_result`
    AS
      SELECT
        *,
        CASE
        ...
        ELSE 'After'
        END AS period
      FROM `WORKFLOW_c3b932854a8512b2_715facbd93094fcc_result`;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_61ff61a040096b87_result`
    AS
      SELECT period,
        COUNT(collision_id) collision_id_count
      FROM `WORKFLOW_c3b932854a8512b2_d38aaa5f086ae0f9_result`
      GROUP BY period;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_43975d37580d7eb8_result`
    AS
      SELECT
        *,
        CASE
        ...
        ELSE 'After'
        END AS period
      FROM `WORKFLOW_c3b932854a8512b2_8b9afe49df58194a_result`;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_a322f9f6cbdd5724_result`
    AS
      SELECT period,
        AVG(value) value_avg
      FROM `WORKFLOW_c3b932854a8512b2_43975d37580d7eb8_result`
      GROUP BY period;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_b5ece8963d58bb57_result`
    AS
      SELECT 
        _main.*,
        _joined.period AS period_joined, _joined.collision_id_count AS collision_id_count_joined
      FROM
        `WORKFLOW_c3b932854a8512b2_f0c6a1fb8fd9b694_result` AS _main
      RIGHT JOIN
        `WORKFLOW_c3b932854a8512b2_61ff61a040096b87_result` AS _joined
      ON
        _main.period = _joined.period;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_088f8c39a25420bc_result`
    AS
      SELECT
      ...
      FROM `WORKFLOW_c3b932854a8512b2_b5ece8963d58bb57_result`;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_c35af1579ebd66a0_result`
    AS
      SELECT 
        _main.*,
        _joined.period AS period_joined, _joined.value_avg AS value_avg_joined
      FROM
        `WORKFLOW_c3b932854a8512b2_088f8c39a25420bc_result` AS _main
      RIGHT JOIN
        `WORKFLOW_c3b932854a8512b2_a322f9f6cbdd5724_result` AS _joined
      ON
        _main.period = _joined.period;
    END;
    BEGIN
    CREATE TEMPORARY TABLE `WORKFLOW_c3b932854a8512b2_81657984f547148c_result`
    AS
      SELECT
        SAFE_CAST(period_joined AS STRING) AS period,
      ...
      FROM `WORKFLOW_c3b932854a8512b2_c35af1579ebd66a0_result`;
    END;
    EXECUTE IMMEDIATE
    REPLACE(
